...
public class Unit2 extends UnitCom {
    Unit2 unit2 = this;

    Unit2(Page p) {
        super(p);
        this.setBackground(new Color(r1, g1, b1));
        this.setBounds((this.getSize().width - this.getSize().height) / 2, 290, p.unit, p.unit * 2);
                ...
                if ((p.getMousePosition() != null) && (p.getMousePosition().x < MainPanel.h) && (p.getMousePosition().y < MainPanel.h)) {
                	if (isDragged == true) {
                        if (p.getComponentAt(p.unitP.x, p.unitP.y).getClass() == p.getClass()) {
                            ...
                            p.add(unit2);
                            unit2.setBackground(new Color(p.r1, p.g1, p.b1));
                            if (isFirst) {
                            	p.units.add(unit2);
                            }
                            p.repaint();
                        ...
                        unit2.drawOutLine(p.getGraphics());
                	}
                } else {
                	if (p.unitP.x != -1) {
                        if (p.getComponentAt(p.unitP.x, p.unitP.y).getClass() == p.getClass()) {
