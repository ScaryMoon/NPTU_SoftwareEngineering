...
public class Unit3 extends UnitCom {
    Unit3 unit3 = this;

    Unit3(Page p) {
        super(p);
        this.setBackground(new Color(r1, g1, b1)); 
        setBounds(((int) p.unitP.x / p.unit) * p.unit + 1, ((int) p.unitP.y / p.unit) * p.unit + 1, p.unit * 2 - 1, p.unit - 1);
                ...
                if ((p.getMousePosition() != null) && (p.getMousePosition().x < MainPanel.h) && (p.getMousePosition().y < MainPanel.h)) {
                	if (isDragged == true) {
                        if (p.getComponentAt(p.unitP.x, p.unitP.y).getClass() == p.getClass()) {
                            ...
                            p.add(unit3);
                            unit3.setBackground(new Color(p.r1, p.g1, p.b1));
                            if (isFirst) {
                            	p.units.add(unit3);
                            }
                            p.repaint();
                        ...
                        unit3.drawOutLine(p.getGraphics());
                	}
                } else {
                	if (p.unitP.x != -1) {
                        if (p.getComponentAt(p.unitP.x, p.unitP.y).getClass() == p.getClass()) {
